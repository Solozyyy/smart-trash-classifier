...
# Colab paths
COLAB_DRIVE_PATH=/content/drive/MyDrive/data.zip
COLAB_DATA_DIR=/content/data_extracted/
COLAB_BALANCED_DIR=/content/balanced_data/
COLAB_TRAIN_DIR=/content/train_data/
COLAB_VAL_DIR=/content/val_data/
COLAB_TEST_DIR=/content/test_data/
COLAB_WEIGHTS_DIR=/content/
...
Xem [SETUP.md](SETUP.md) và [README_CONFIG.md](README_CONFIG.md) để biết thêm chi tiết.
...
### **Q: ResNet18 không có trong Keras?**
A: Đúng vậy. `tf.keras.applications` không có ResNet18. Project này dùng **ResNet50** thay thế, performance tốt hơn nhưng chậm hơn 1 chút.

### **Q: Data.zip không tìm thấy?**
```python
# Kiểm tra đường dẫn
...
### **Q: Training quá chậm?**
- Đảm bảo đang dùng GPU trên Colab: Runtime → Change runtime type → GPU (T4)
- Kiểm tra: `tf.config.list_physical_devices('GPU')`

### **Q: Làm sao download plots về?**
Cell cuối cùng trong notebook tự động:
```python
shutil.make_archive('evaluation_plots', 'zip', 'evaluation_plots')
files.download('evaluation_plots.zip')
```
...
- [Best Practices for Image Classification](https://developers.google.com/machine-learning/practica/image-classification)
